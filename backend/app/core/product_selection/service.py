...
class ProductSelectionService:
    ...
    async def list_selections(
        ...
        if store_ids:
            query = query.where(ProductSelection.store_id.in_(store_ids))
            count_query = count_query.where(ProductSelection.store_id.in_(store_ids))
        if status:
    ...
    async def get_selection_stats(self, store_ids: list[int] | None = None) -> dict:
        query = select(ProductSelection)
        if store_ids:
            query = query.where(ProductSelection.store_id.in_(store_ids))
