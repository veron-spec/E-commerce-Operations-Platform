...
class SuggestionService:
    ...
    async def list_suggestions(
        ...
        if store_ids:
            query = query.where(Suggestion.store_id.in_(store_ids))
            count_query = count_query.where(Suggestion.store_id.in_(store_ids))
        if suggestion_type:
    ...
    async def get_suggestion_stats(self, store_ids: list[int] | None = None) -> dict:
        query = select(Suggestion)
        if store_ids:
            query = query.where(Suggestion.store_id.in_(store_ids))
